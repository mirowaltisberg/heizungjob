...
export function classifyHeizungTitle(titleValue: unknown): JobSafetyResult {
  ...
  if (!match) return { disposition: "REJECT", reason: "no-heizung-title-signal" };
  ...
  return {
    disposition: "ACCEPT",
    reason: specific ? "specific-heizung-title" : "broad-role-with-heizung-context",
...
export function isValidHeizungJobId(value: unknown): value is string {
  return typeof value === "string" && idPattern.test(value);
}

export function getHeizungProfile(profile: JobSafetyProfile) {
