...
import { classifyHeizungTitle, isValidHeizungJobId } from "@/lib/job-safety";
...
function scoreScrapedJob(job: ScrapedJob): number {
  return classifyHeizungTitle(job.title).disposition === "ACCEPT" ? 10 : -100;
}
...
export async function getJobListingById(input: {
  ...
  if (!isValidHeizungJobId(input.id)) {
    return null;
  }
