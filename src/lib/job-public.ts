...
import { classifyHeizungTitle, getHeizungProfile } from "@/lib/job-safety";
...
export function buildPublicJobCopy(input: PublicJobCopyInput): PublicJobCopy {
  ...
  const classification = classifyHeizungTitle(input.title);
  if (
  ...
  const profile = getHeizungProfile(classification.profile);
