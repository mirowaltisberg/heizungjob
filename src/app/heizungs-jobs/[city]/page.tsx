...
import { HEIZUNG_CITIES, findHeizungCity } from "@/lib/heizung-cities";
...
const ROLE_LINKS: { slug: string; label: string }[] = [
  { slug: "heizungsinstallateur-efz", label: "Heizungsinstallateur EFZ" },
  { slug: "heizungsmonteur-efz", label: "Heizungsmonteur EFZ" },
  { slug: "waermepumpen-techniker", label: "Wärmepumpen-Techniker" },
  { slug: "servicetechniker-heizung", label: "Servicetechniker Heizung" },
  { slug: "projektleiter-heizung", label: "Projektleiter Heizung" },
];

export async function generateStaticParams() {
  return HEIZUNG_CITIES.map((c) => ({ city: c.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { city: citySlug } = await params;
  const city = findHeizungCity(citySlug);
  if (!city) return { title: "Heizungsinstallateur Jobs" };

  const title = `Heizungsinstallateur Jobs ${city.name} 2026 | Stellen für Heizungsinstallateur in ${city.name}`;
  const description = `Offene Heizungsinstallateur Stellen in ${city.name} (${city.cantonAbbr}). Lohn ${city.salaryBand}, tägliche Updates aus der ${city.region}.`;
...
export default async function HeizungCityPage({ params }: PageProps) {
  const { city: citySlug } = await params;
  const city = findHeizungCity(citySlug);
  if (!city) notFound();
  ...
  const itemListSchema = {
    ...
    "@type": "ItemList",
    name: `Heizungsinstallateur Jobs ${city.name}`,
    description: `Offene Heizungsinstallateur Stellen in ${city.name}`,
  ...
  const FAQS = [
    {
      question: `Wie viele Heizungsinstallateur Stellen gibt es aktuell in ${city.name}?`,
      answer: `In ${city.name} und Umgebung sind regelmässig zwischen 20 und 120 offene Heizungsinstallateur Stellen ausgeschrieben. Die Zahl schwankt mit Saison, Bautätigkeit und Auftragslage. Pendelbereite Bewerbende erweitern den Suchradius typischerweise auf ${city.commuterTowns.slice(0, 3).join(", ")} und finden so 2- bis 3-mal mehr passende Stellen.`,
    },
    {
      question: `Was verdient ein Heizungsinstallateur in ${city.name}?`,
      answer: `Ein Heizungsinstallateur verdient in ${city.name} typischerweise ${city.salaryBand} pro Jahr (12 × Monatslohn). Berufseinsteiger starten am unteren Ende, mit fünf Jahren Erfahrung und Spezialisierungen verschiebt sich der Lohn deutlich nach oben. Detaillierte Vergleichstabellen findest du auf der Lohnübersicht.`,
    },
    {
      question: `Welche Arbeitgeber suchen Heizungsinstallateur in ${city.name}?`,
    ...
    {
      question: `Wie pendle ich am besten zu einem Heizungsinstallateur Job in ${city.name}?`,
      ...
      <main className="bg-white">
        <section className="bg-primary/5 border-b">
          <div className="container mx-auto px-4 sm:px-6 py-10 sm:py-14 max-w-5xl">
            <nav className="text-sm text-slate-500 mb-3" aria-label="Breadcrumb">
              ...
              <span className="text-slate-700">Heizungsinstallateur Jobs {city.name}</span>
            </nav>
            <h1 className="text-3xl sm:text-5xl font-black text-slate-900 leading-tight mb-4">
              Heizungsinstallateur Jobs <span className="text-primary">{city.name}</span>
            </h1>
        ...
        <section className="container mx-auto px-4 sm:px-6 py-10 max-w-5xl">
          <h2 className="text-2xl font-bold text-slate-900 mb-4">
            Offene Heizungsinstallateur Stellen in {city.name}
          </h2>
                ...
                <Card key={`${job.source}-${job.id}`} className="hover:border-primary/40 transition">
                  <CardContent className="p-4">
                    ...
                    <p className="text-sm text-slate-600 mb-2 line-clamp-1">
                      {job.company || "Schweizer Heizungsbetrieb"}
                    </p>
        ...
        <section className="bg-slate-50 border-y">
          <div className="container mx-auto px-4 sm:px-6 py-10 max-w-5xl">
            <h2 className="text-2xl font-bold text-slate-900 mb-4">
              Heizungsinstallateur Berufe in {city.name}
            </h2>
        ...
        <section className="bg-slate-50 border-t">
          <div className="container mx-auto px-4 sm:px-6 py-10 max-w-4xl">
            <h2 className="text-2xl font-bold text-slate-900 mb-5">
              Häufig gestellte Fragen — Heizungsinstallateur Jobs {city.name}
            </h2>
        ...
        <section className="container mx-auto px-4 sm:px-6 py-10 max-w-4xl">
          ...
          <div className="flex flex-wrap gap-2">
            { HEIZUNG_CITIES.filter((c) => c.slug !== city.slug).map((c) => (
              <Link
                key={c.slug}
              ...
              >
                Heizungsinstallateur Jobs {c.name}
              </Link>
            ))}
          </div>
        </section>
