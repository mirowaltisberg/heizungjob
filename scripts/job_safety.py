"""Shared deny-first Heizung classifier backed by src/config/job-safety.json."""
...
def classify_heizung_title(title_value: Any) -> dict[str, Any]:
    ...
    if not match:
        return {"disposition": "REJECT", "reason": "no-heizung-title-signal"}
    ...
    return {
        "disposition": "ACCEPT",
        "reason": "specific-heizung-title" if specific else "broad-role-with-heizung-context",
